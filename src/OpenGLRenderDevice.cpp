...
int OpenGLRenderDevice::render(Renderable& r) {
	...
	if (0 == r.texture) {
		...
		glBegin(GL_QUADS);
		glTexCoord2f(r.gl_src[0], r.gl_src[1]);
		glVertex2f(m_x0, m_y0);
		glTexCoord2f(r.gl_src[2], r.gl_src[1]);
		glVertex2f(m_x1, m_y0);
		glTexCoord2f(r.gl_src[2], r.gl_src[3]);
		glVertex2f(m_x1, m_y1);
		glTexCoord2f(r.gl_src[0], r.gl_src[3]);
		glVertex2f(m_x0, m_y1);
		glEnd();
	}
...
bool OpenGLRenderDevice::local_to_global(Renderable& r) {
	...
	if (r.local_frame.w) {
		...
		m_clip.w = right - left;
		r.gl_src[0] = (float)m_clip.x/r.sprite->w;
		r.gl_src[2] = r.gl_src[0] + (float)m_clip.w/r.sprite->w;
	}
	if (r.local_frame.h) {
		...
		m_clip.h = down - up;
		r.gl_src[1] = (float)m_clip.y/r.sprite->h;
		r.gl_src[3] = r.gl_src[1] + (float)m_clip.h/r.sprite->h;
	}

	m_x0 = (float)left + r.local_frame.x;
	m_y0 = (float)up + r.local_frame.y;
	m_x1 = (float)right + r.local_frame.x;
	m_y1 = (float)down + r.local_frame.y;
